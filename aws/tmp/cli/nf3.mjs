...
function createQueue(options) {
  ...
  queue.on('error', error => logger.error({ msg: 'Nf3 - Error caught by queue', error }));
...
class Nf3 {
  ...
  // min fee or reward one should hold for withdrawing
  // in State contract.
  ...
  constructor(
    ...
    environment = {
      ...
      optimistWsBpUrl: 'ws://localhost:3041',
    ...
    this.optimistBaseBpUrl = environment.optimistApiBpUrl;
    this.optimistWsUrl = environment.optimistWsUrl;
    this.optimistWsBaUrl = environment.optimistWsBaUrl;
    this.optimistWsBpUrl = environment.optimistWsBpUrl;
    ...
    this.applyHttpClientAuthentication(this.clientBaseUrl);
    this.applyHttpClientAuthentication(this.clientBaseBpUrl);
    this.applyHttpClientAuthentication(this.clientBaseTxUrl);
  }

  applyHttpClientAuthentication(url) {
  ...
  /**
    Initialises the Nf3 object so that it can communicate with Nightfall_3 and the
    blockchain.
    @returns {Promise}
    */
  ...
  /**
    ...
    This will also update the corresponding Ethereum address that Nf3 uses.
    @method
  ...
  async estimateGasPrice() {
    ...
    proposedGasPrice = isNaN(proposedGasPrice) ? GAS_PRICE : proposedGasPrice;
  ...
  */
  async submitTransaction(unsignedTransaction, contractAddress = this.shieldContractAddress, fee) {
    ...
    logger.debug(`Sending transaction with L1 hash ${tx.transactionHash}`);
  ...
  */
  ...
  async deposit(
    ...
    return new Promise((resolve, reject) => {
      userQueue.push(async () => {
        try {
          logger.debug({ msg: 'Deposit transaction being processed', l2Tx: res.data.transaction });
      ...
      return new Promise((resolve, reject) => {
        userQueue.push(async () => {
          try {
            ...
            resolve({ ...receipt, transaction: res.data.transaction });
          } catch (err) {
            reject(err);
          }
        });
  ...
  /**
    Post incoming viewing key API call
    @method
    @async
    */
  async postIncomingViewingKeys() {
    const res = await axios.post(`${this.clientBaseBpUrl}/incoming-viewing-key`, {
  ...
  async subscribeToIncomingViewingKeys() {
    let res = await this.postIncomingViewingKeys();
    // prevent commitment decryption while client is in
    // syncing state to avoid race conditions
    while (res.data.status === 'syncing') {
      logger.debug('Client syncing, please wait...');
      // eslint-disable-next-line no-await-in-loop
      await new Promise(resolve => setTimeout(resolve, 100));
      // eslint-disable-next-line no-await-in-loop
      res = await this.postIncomingViewingKeys();
    }
    return res;
  }

  /**
    Closes the Nf3 connection to the blockchain and any open websockets to Nf3
    @method
    */
  close() {
  ...
  async changeCurrentProposer() {
    const res = await axios.post(`${this.optimistBaseUrl}/proposer/change`);
    return this.submitTransaction(res.data.txDataToSign, this.stateContractAddress);
  }
  ...
  /**
    ...
    */
  async withdrawStake() {
    const res = await axios.post(`${this.optimistBaseUrl}/proposer/withdraw-stake`);
  ...
  startPeriodicPayment(cronExp = '0 0 * * */6') {
    this.periodicPaymentJob = createJob(cronExp, async () => {
      try {
        ...
        const res = await axios.post(`${this.optimistBaseUrl}/proposer/withdraw`);
        logger.info({ msg: 'withdraw finished', receipt: res.data.receipt });
      } catch (err) {
        logger.error({
  ...
  async startChallenger() {
    ...
    const connection = new ReconnectingWebSocket(this.optimistWsBpUrl, [], { WebSocket });
  ...
  challengeEnable(enable) {
    return axios.post(`${this.optimistBaseBpUrl}/challenger/enable`, { enable });
  }
  ...
  /**
    ...
    value of each property is the number of tokens originating from that contract.
    */
