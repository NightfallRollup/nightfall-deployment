...
#  Usage
#  AWS_ACCESS_KEY_ID=<xxxx> AWS_SECRET_ACCESS_KEY=<xxxxxxxxxx> RELEASE=<xxxx> ./status-task <task-name>
...
if  [ "$#" -ne 1 ]; then
  echo "Usage: AWS_ACCESS_KEY_ID=<xxxx> AWS_SECRET_ACCESS_KEY=<xxxxxxxxxx> ./status-task <task-name>"
  exit 1
fi
...
for TASK_ARN in ${TASKS_ARN}; do
  ...
  if [ "${TASK_NAME}" ]; then
      ...
      TASK_FOUND=1
      # task status
      echo "Task's status - ${TASK_NAME}: $TASK_STATUS"
      # Task is not Running
