...
#  Stops MongoDb on AWS (non blocking)

#  Usage
#  AWS_ACCESS_KEY_ID=<xxxx> AWS_SECRET_ACCESS_KEY=<xxxxxxxxxx> RELEASE=<xxxx> ./stop-db.sh
#
set -e
...
if [ "${DOCDB_STATUS}" == "available" ]; then
   aws docdb stop-db-cluster --db-cluster-identifier ${MONGO_ID} > /dev/null
else
